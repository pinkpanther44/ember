...
public:
    ...
    /** 録音ファイルの保存先フォルダ（仕様書9章「外部パス参照のみ」の方針に沿う）。
        ...
        8.286：**開いているプロジェクトのフォルダの中（`Rec`）**を返します
        （Phase 279／本人の要望）。まだ保存していないプロジェクトでは、
        環境設定の逃げ場を返します——ただし**未保存では録音の前に保存を促す**ので
        （`MainComponent`）、そちらへ落ちるのはフォルダが作れないときだけです。

        **staticではなくなりました**：どのプロジェクトを開いているかで答えが変わるためです。 */
