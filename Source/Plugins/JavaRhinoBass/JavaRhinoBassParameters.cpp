...
namespace JavaRhinoBassParams
{
    ...
    juce::AudioProcessorValueTreeState::ParameterLayout createParameterLayout()
    {
        ...
        // **既定は0.30**（`JBass5`は0.80）。
        //
        // あちらは**後ろに常時コンプとアンプ段**がありました。それを持って来ていないので、
        // 弦の素の大きさがそのまま出ます——**実測で峰が1.6〜2.2**（ベロシティ0.9、
        // いちばん大きいのはG4とスラップ）。0.80のままだと、弾いただけで0dBFSを超えます。
        //
        // 8.269：**0.35では足りていませんでした**（Phase 271）。
        //
        // Phase 266で0.35にしたとき、ここには「峰は0.8前後に収まる」と書きました。
        // **1回測って書いた数字でした。** 乱数の種がそのつど違っていたので
        // （8.269の再現性の話）、**引きが良かった回を見ていた**ということです。
        //
        // 種を固定してから測り直すと：
        //
        // | | 0.35のとき | 0.30のとき |
        // |---|---|---|
        // | G4 | **1.034**（超えている） | 0.886 |
        // | スラップA1 | 0.996 | 0.854 |
        // | 参考：Racco GuitarのE2 | 0.775 | — |
        //
        // **0.3dBしか余裕が無い状態**だったので、どこかを触るたびに超えます。
        // 0.30にして、ようやく「峰は0.8前後」が本当になりました。
        //
        // **保存済みのプロジェクトは動きません**（保存された値が優先されます）。
        // 変わるのは**新しく挿したとき**だけです。
        layout.add (std::make_unique<Float> (
            juce::ParameterID { gain, 1 }, "Output",
            juce::NormalisableRange<float> (0.0f, 1.5f, 0.001f), 0.30f));
