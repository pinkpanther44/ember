    ...
    */
    ...
    public:
        ...
        void setSettings (const Settings& newSettings)
        {
            ...
            // 8.224：**上限12→28、配り方は指数**（Phase 247／本人の要望
            // 「Driveの係数ももう少し上げてほしい」）。
            //
            // **フィードバックの中の音は、そもそも小さい**のが効きの薄かった理由です。
            // Feedback 35%なら1回目の反復は入力の0.35倍で、そこからさらに減っていきます——
            // `g = 12`でも`tanh`の肩にほとんど届きません（入力0.1なら`tanh(1.2)`で-3dB程度）。
            //
            // | Drive | 前（線形1→12） | いま（指数1→28） |
            // |---|---|---|
            // | 25% | 3.8 | 2.3 |
            // | 50% | 6.5 | 5.3 |
            // | 75% | 9.3 | 12.2 |
            // | 100% | 12 | **28** |
            //
            // **真ん中あたりは前とほぼ同じ**で、上だけ伸びます——
            // 線形のまま上限だけ上げると、**低いところが一気に効きすぎ**ます。
            driveGain = (capabilities.drive && settings.drive > 0.0001f)
                          ? std::pow (28.0f, settings.drive)
            ...
            //
            // 8.224：**だからDriveを上げるほど音は小さくなります**（Phase 247）。
            // `tanh(g·x)/g`は、大きい信号に対しては`1/g`へ近づくためです。
            // **これは直せません**——もっと戻すと、`1/tanh(g)`でやった発振に戻ります。
            // 音量が要るときは、そのエンジンの`Level`か`Output`で上げること。
