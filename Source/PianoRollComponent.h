...
private:
    ...
    /** 8.276：**掴める範囲**（Phase 274／本人の報告）。

        ### ドラムでは、描いてある▶そのもの

        ドラム画面のノートは**長さを描いていません**（8.121で尾を落としました）。
        それなのに**当たり判定だけが長さの矩形のまま**だったので、
        1音が長いと**次の音の▶の上まで自分の判定が伸びます**。
        重なったところでは**先に見つかったほう＝手前の音**が拾われるので、
        「クリックした1つ前が選ばれる」という形で出ます
        （1/16で並べると必ず起きます。本人の報告どおり）。

        **見えているものを掴ませること。** ドラムでは▶の四角だけを返します。

        ピアノロールでは長さが描いてあるので、今までどおり矩形そのものです。 */
    juce::Rectangle<int> getNoteHitBounds (const Note& note) const;
    ...
    /** 他トラックのノートの透かし（Phase 73）。**塗りは薄く**：
        ...
        8.305：**0.22から0.38へ**（Phase 298／本人の指定「もう少し濃く」）。

        Phase 73の0.22は「編集中のノートと紛れないこと」だけを見て決めた値でした。
        **透かしは見るためにあります**——薄すぎると、点けたのに
        「どこに何があるか」が読めず、点ける意味がありません。

        > **上限は「編集できるように見えないこと」。** 透かしは掴めないので、
        > 本体と同じ濃さになると**掴めないノート**に見えます（8.161）。
        > 本体は枠付きで塗りつぶし、こちらは枠なしなので、
        > 0.38でも「奥にあるもの」として読めます。 */
    static constexpr float watermarkAlpha = 0.38f;
...
private:
    ...
    /** 8.275：ドラッグでの長さ変更。**掴んだノート以外**の選択中のノートも、
        同じだけ伸び縮みさせる（Phase 274／本人の要望）。

        ### 同じ長さにせず、同じだけ伸ばす

        ベロシティと同じ考え方です（Phase 127）——**足すのは差分**。
        選んだノートの長さを全部そろえてしまうと、
        **打ち込んだリズムの形が1回のドラッグで潰れます**（付点や刻みの混ざった
        フレーズを選んでいると、戻すにはUndoしかありません）。
        長さがもともと揃っていれば、差分でも結果は同じです。

        **左端のドラッグでは、始まりも動かします。** 終わりは動かさないので、
        動いた量ぶんだけ長さが増減します（掴んだ1つと同じ振る舞い）。

        **区切り（beginAction）は呼び出し元が作っておくこと**
        （`moveOtherSelectedNotesByDrag()`と同じ）。 */
    void resizeOtherSelectedNotesByDrag();
    ...
    //==========================================================================
    /** 8.305：**掴んだまま画面の外まで行ける**（Phase 298／本人の指定）。

        > 「ノートをドラッグして移動・複製・選択などする際、表示されている
        > 画面外へも移動・複製・選択ができるように、**マウスを追いかけて
        > 画面を自動でスクロールする**ような仕様を取り入れよう」

        Phase 297までは、見えている範囲の外へは持って行けませんでした——
        一度離して、スクロールして、掴み直す。**選択範囲は掴み直せない**ので、
        画面をまたぐ範囲選択は作れません。

        ### 仕組み

        `juce::Component::beginDragAutoRepeat()`で、**マウスが止まっていても
        `mouseDrag()`が呼ばれ続ける**ようにしてあります。1回呼ばれるたびに
        少しスクロールし、**そのまま下の処理へ落とす**ので、
        行き先はいつも「いまのカーソルの位置」から決まります
        （スクロールぶんを別に足し込む必要がありません）。

        ### 縦と横で、直すものが違います

        | | スクロールするもの | 掴んだ位置の直し |
        |---|---|---|
        | 縦 | **ビューポート**（この部品ごと動く） | **要りません** |
        | 横 | **この部品の中**（`scrollStartSeconds`） | **要ります** |

        縦は部品ごと動くので、**部品座標で覚えた起点は中身に貼り付いたまま**です。
        横は部品の位置が変わらず中身だけ流れるので、
        **起点のXを同じだけずらさないと、掴んだ場所が中身からはがれます**
        （`shiftDragAnchorsX()`）。 */
    ...
    /** 1回ぶんのスクロール量。**外へ出たぶんに比例**させて、上限で止める。
        ...
        止めないと、画面から大きく外した瞬間に曲の端まで飛びます。 */
    static constexpr int autoScrollMaxStepPixels = 24;
