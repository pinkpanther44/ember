...
private:
    ...
    /** 仕様書5.1：前回のオートセーブが残っていれば、復元するか確認する。
        ...
        8.320：`then`は**答えが出てから**呼ばれます（残っていなければすぐ）。
        起動時の確認を**1つずつ順に**出すためです——同時に出すと窓が重なります。 */
    void offerAutoSaveRecovery (std::function<void()> then = {});

    /** 8.320：**前回、本体が落ちていたら**、記録が残っていることを知らせる（Phase 312）。

        落ちたときの記録（`CrashLog`）は、**知らせなければ誰も見ません**。
        知らせるのは**まだ見ていないものがあるときだけ**、1回だけです。 */
