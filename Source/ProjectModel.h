...
public:
    ...
    /** 8.279：**重なっているぶんだけ短くする**（Phase 275／本人の指定）。

        ### Phase 274までは「隙間なく並べ直す」でした（8.128）

        旗で表すことにしたとき（Phase 164）、**長さは「次の旗まで」で自動**と決めました。
        自分で長さを決められない代わりに、**隙間ができない**——という組み合わせです。

        本人の要望は「コード区間の長さを変えられるようにしてほしい。
        ピアノロールの帯と同じように」でした。**どちらか一方しか選べません**：

        | | 長さを決められる | 隙間 |
        |---|---|---|
        | Phase 274まで | できない | **できない**（次の旗まで自動で伸びる） |
        | **いま** | **できる** | **できる**（短くしたぶんはコードが無い） |

        本人が選んだのは**下**です。そのため、この関数は
        **伸ばすことをしません**——重なっている区間を、次の旗の手前まで**縮めるだけ**。

        | 何をしたか | どうなるか |
        |---|---|
        | 旗を右へ動かした | 手前の区間はそのまま＝**隙間ができる** |
        | 旗を左へ動かした | 手前の区間が重なるので**縮む** |
        | 旗を消した | 手前の区間はそのまま＝**そこが隙間になる** |

        **いちばん後ろの区間は触りません**（次が無いので、曲の終わりはそちらが決める）。

        **区間を足す・動かす・消したあとは必ず呼ぶこと。** 呼び忘れると
        **区間どうしが重なり**、`findChordRegionAt()`が先に見つけたほうを返します
        （どちらのコードなのかが場所によって変わる、という読めない状態になります）。 */
    void trimOverlappingChordRegions (juce::UndoManager* undoManager);
...
public:
    ...
    /** 8.305：**目盛りに加えて、`referenceSeconds`のずれを保った位置にも寄せる**
        （Phase 298／本人の指定）。

        ### 何のためにあるか

        目盛りから外れたところに置いたノート（グルーヴ、人力のもたり）を
        Snapを入れたまま動かすと、**目盛りにしか止まりません**。
        元と同じタイミングの別の音を作る、1小節あとへ同じ形で写す——
        どちらも**Snapを切って手で合わせる**しかありませんでした
        （本人の動画：コピーが`3.01.01.00`と`3.02.01.00`を行き来して、
        元の音の上に置けない）。
        ...
        |---|---|
        | 目盛り | `snapTime()`と同じ |
        | **ずれを保った位置** | `referenceSeconds`の「目盛りからのずれ」を足した目盛り |

        **近いほうへ寄ります。** 元が目盛りの上にあるノート（ずれ0）では
        2つが同じ値になるので、**今までと何も変わりません**。

        ずれは**拍で測ります**。秒で覚えると、テンポの変化点をまたいだときに
        ずれの大きさが変わります（8.98）。 */
    double snapTimeRelativeTo (double seconds, double referenceSeconds) const;
