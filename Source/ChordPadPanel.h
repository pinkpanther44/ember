...
private:
    ...
    /** 書き込み先の候補（MIDIトラック）を並べ直す。トラックが増減すると変わる。
        ...
        **いま選ばれている書き込み先は保ちます**（IDで覚え直す）。
        選び直すのは`followSelectedTrack()`のほうの仕事です。 */
    void refreshTargetTrackList();

    //==========================================================================
    /** 8.272：**選んでいるトラックへ書き込み先を合わせる**（Phase 272／本人の要望）。

        ### なぜタブを残したまま追従させるのか

        Phase 271まで、書き込み先は**このパネルの中だけで選ぶもの**でした。
        アレンジ画面でベースのトラックを選んで、コードパッドで弾いてみると、
        **さっきまで触っていたトラックとは別のところへ音が入ります**。
        「どこへ入るのか」を覚えていなければならないのが面倒、というのが本人の言葉です。

        | 何をしたか | 書き込み先 |
        |---|---|
        | MIDIトラック（かその上のクリップ）を選んだ | **そのトラックへ移る** |
        | オーディオ／コードトラックを選んだ | **そのまま**（MIDIではないので書き込めない） |
        | タブから自分で選んだ | そのトラックへ移る（次にトラックを選ぶまで） |

        **タブは残してあります。** 画面を切り替えずに書き込み先だけ変えたいことが
        あるためで、どちらから変えても同じ場所へ入ります。

        **呼ぶのは`refreshFromModel()`と選択の通知から。**
        `refreshTargetTrackList()`の中では呼びません——あちらは
        トラック名が変わっただけでも走るので、
        **タブで選び直したものが、名前を直した拍子に戻ります**。 */
