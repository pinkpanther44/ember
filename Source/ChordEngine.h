...
namespace Voicing
{
    /** 生成するノートの音域。
        ...
        **ピアノロールより広くしないこと。** ここを外れた音は捨てられるので、
        広げると「鳴っているのに画面に出ない・掴めないノート」ができます（HANDOVER 1.9）。

        8.282：**同じ値である必要はなくなりました**（Phase 275）。
        ピアノロールはMIDIの全域（0〜127）を出すようになったので、
        こちらは**作る音の範囲**として、これまでの値のままです
        ——広げると、オクターブを下げたときのベース音が可聴域の下へ落ちていきます。
